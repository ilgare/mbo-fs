...
# feature selection performance on the glioma detection dataset:
#
...
# Load dataset
X = pd.read_csv('glioma/X.csv')
X = X.iloc[:,1:]
y = pd.read_csv('glioma/y.csv')

# The "Race" column is string, convert it to integers
...
now = datetime.now()
output_fname=f"output_glioma_"+now.strftime("%d-%m-%y_%H:%M:%S")+".txt"
