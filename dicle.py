...
# feature selection performance on the mesothelioma disease (Dicle) dataset:
#
# https://doi.org/10.24432/C5903T
...
# Load dataset
data = pd.read_excel("mdset.xlsx")
data.rename(columns={'class of diagnosis': 'target'}, inplace=True)
data.to_excel(r"new_excel_file.xlsx", index=False)

# Convert class ids 1-2 to 0-1
y=np.array(data.target - 1)

filt = [ (i!=6 and i!=34) for i in range(0,35)]

X1 = data.iloc[:,filt]
X=np.asarray(X1)

# Feature 6 seems to be determining the output
#for i in range(0,33):
#    cm = np.corrcoef(X[:,i],y)
#    print(f"{i}   {cm[0,1]}")
#exit()
...
now = datetime.now()
output_fname=f"output_dicle_"+now.strftime("%d-%m-%y_%H:%M:%S")+".txt"
